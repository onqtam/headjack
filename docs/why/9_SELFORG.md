...
One way to think about relevance realization when implementing something like it digitally in terms of mechanisms & algorithms: ranking/sorting - you don't necessarily need to rank/sort all elements (that might take too much time) but you should be able to at least answer queries of the sort: `"What are the top X elements"` and optionally get the result with weights (quantitative comparison) and not just as simple positional ordering.
